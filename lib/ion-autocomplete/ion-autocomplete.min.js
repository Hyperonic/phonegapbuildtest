...
angular.module('ion-autocomplete', []).directive('ionAutocomplete', [
    '$ionicBackdrop', '$ionicScrollDelegate', '$document', '$q', '$parse', '$interpolate', '$ionicPlatform', '$compile', '$templateRequest', '$rootScope',
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest, $rootScope) {
        return {
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    '<div class="bar item-input-inset search-bar nomargin">',
                    '<label class="item-input-wrapper">',
                    '<i class="ft25 icon ion-search placeholder-icon"></i>',
                    ...
                    '</label>',
                    '<span ng-click="viewModel.clearInput()" ng-class="{\'ion-ios-close\' : viewModel.searchQuery}" class="button button-clear ft25"></span>',
                    '</div>',
                    ...
                    '</div>',
                    '<ion-content class="custom-radio has-fixedbtn has-header">',
                    '<div ng-show="viewModel.selectedItems.length > 0" class="list list-inset cont-list">',
                    '<ion-item class="modal-list-items search-bar">{{viewModel.selectedItemsLabel}}</ion-item>',
                    '<ion-list>',
                    ...
                    '</ion-list>',
                    '</div>',
                    '<div ng-show="viewModel.searchItems.length > 0" class="list list-inset cont-list">',
                    '<ion-item class="modal-list-items search-bar" ng-if="viewModel.searchItems.length > 0">{{viewModel.selectItemsLabel}}</ion-item>',
                    '<ion-list>',
                    ...
                    '</div>',
                    '</ion-content>',
                    '<div class="fixedbtn custom-radio row text-center">',
                      '<div class="col">',
                          '<button btnfunc="viewModel.cancelClick()" class="icon-right ion-chevron-right tooltip register-btn button app-btn yellow">{{viewModel.cancelLabel}}</button>',
                      '</div>',
                    '</div>',
                    '<custom-footer ng-if="shown"/>',
                    '</div>'
                ].join('');
                ...
                $q.when().then(function () {
                    ...
                    ionAutocompleteController.showModal = function () {
                        ...
                        if(scope.viewModel.selectedItems.length == 0){
                            searchInputElement[0].focus();
                            setTimeout(function () {
                                searchInputElement[0].focus();
                                if(ionic.Platform.isWebView() && !cordova.plugins.Keyboard.isVisible) cordova.plugins.Keyboard.show();
                            }, 500);
                        }
                        $rootScope.clearNotifIntv();
                    ...
                    ionAutocompleteController.clearInput = function(){
                    	scope.viewModel.searchQuery = '';
                    }
